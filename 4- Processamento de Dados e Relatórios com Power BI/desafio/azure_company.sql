...
-- Verifique a estrutura da tabela 'departament'
SHOW CREATE TABLE departament;

ALTER TABLE departament ADD CONSTRAINT fk_dept_old FOREIGN KEY (Mgr_ssn) REFERENCES employee (Ssn) ON UPDATE CASCADE;

-- Remova a restrição antiga (se existir)
ALTER TABLE departament DROP FOREIGN KEY fk_dept_old;

-- Adicione a nova restrição 'fk_dept'
...
-- Verifique a estrutura da tabela 'dept_locations'
SHOW CREATE TABLE dept_locations;

-- Se a restrição 'fk_dept_locations' já existir, renomeie-a
ALTER TABLE dept_locations ADD CONSTRAINT fk_dept_locations_old FOREIGN KEY (Dnumber) REFERENCES departament (Dnumber)
    ON DELETE CASCADE
    ON UPDATE CASCADE;

-- Remova a restrição antiga (se existir)
ALTER TABLE dept_locations DROP FOREIGN KEY fk_dept_locations_old;

-- Adicione a nova restrição 'fk_dept_locations'
ALTER TABLE dept_locations ADD CONSTRAINT fk_dept_locations FOREIGN KEY (Dnumber) REFERENCES departament(Dnumber)
	on delete cascade
    on update cascade;
